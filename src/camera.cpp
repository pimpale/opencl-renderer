...
camera :: camera(float x, float y, float z, int x_size, int y_size)
{
	...
	kernel = cl::Kernel(build_program(raycast_3d_simple,context,default_device),"raycast");
...
void camera :: get_next_frame()
{
	...
	queue.enqueueReadBuffer(buffer_col_array,CL_TRUE,0,col_array_size,col_array);

	queue.enqueueWriteBuffer(buffer_loc_array,CL_TRUE,0,loc_array_size,loc_array);
	queue.enqueueWriteBuffer(buffer_vec_array,CL_TRUE,0,vec_array_size,vec_array);
	queue.enqueueNDRangeKernel(kernel,cl::NullRange,cl::NDRange(ray_num),cl::NullRange, NULL, &event);
