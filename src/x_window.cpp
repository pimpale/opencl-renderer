...
x_window ::x_window(uint32_t x_size, uint32_t y_size) {
  ...
  this->x_size = x_size;
  this->y_size = y_size;
  println(std::string("Attempting to initialize x window with dimensions: ") +
          std::to_string(x_size) + std::string("x") + std::to_string(y_size));
  XInitThreads();
  dis = XOpenDisplay((char *)0);
  win = XCreateSimpleWindow(dis, DefaultRootWindow(dis), 0, 0, x_size, y_size,
                            0, 0, 0);
  XSetStandardProperties(dis, win, "Now with even more confusing vectors!",
                         "4Sight", None, NULL, 0, NULL);
...
void x_window ::get_user_input() {
  ...
  while (XPending(dis) > 0) {
    ...
    case KeyRelease: {
      KeySym k = XLookupKeysym(&event.xkey, 0);
      keys_pressed.erase(
          std::remove(keys_pressed.begin(), keys_pressed.end(), k),
          keys_pressed.end());
    } break;
    case ButtonPress: {
      // mouse is down
...
bool x_window ::is_key_pressed(KeySym key_sym) {
  return (std::find(keys_pressed.begin(), keys_pressed.end(), key_sym) !=
          keys_pressed.end());
}

void x_window ::redraw() {}

void x_window ::set_color(uint32_t color) { XSetForeground(dis, gc, color); }
...
void x_window ::fill_rect(uint32_t x, uint32_t y, uint32_t width,
                          uint32_t height) {
