...
cl::Platform get_platform(int32_t i, bool display) {
	...
	if(display)
	{
		...
		{
			std::cout << "Platform " << j << ": " <<all_platforms[j].getInfo<CL_PLATFORM_NAME>() <<std::endl;
		}
	}
...
cl::Device get_device(cl::Platform platform, int32_t i, bool display) {
	...
	if (display) {
		...
		{
			std::cout << "Device " << j << ": " <<all_devices[j].getInfo<CL_DEVICE_NAME>() <<std::endl;
		}
	}
...
cl::Program build_program(std::string program_source, cl::Context context, cl::Device device)
{
