...
void get_user_input(x_window *win, camera *cam) {
  while (running) {
    ...
    if (camera_x_size != cam->get_x_size() ||
        camera_y_size != cam->get_y_size()) {
...
int main() {
  // println(std::string("began execution successfully at: ") +
  // executable_path());

  println("starting initialization procedures");
  camera cam(0, 0, -10, DEFAULT_FRAME_XSIZE / SCALE,
             DEFAULT_FRAME_YSIZE / SCALE);
